...
app.post('/api/admin/reset-password', async (req, res) => {
  ...
  // Allow subadmin to reset their own password
  try {
    const subadmin = await db.getSubadminByUsername(normalizedUsername);
    if (!subadmin) {
    ...
    await db.updateSubadminPassword(normalizedUsername, new_password);
...
app.post('/api/admin/reset-username', async (req, res) => {
  ...
  if (password === ADMIN_PASSWORD) {
    ...
    return res.json({ success: true, message: 'Admin username updated successfully' });
  }

  // Check if password matches a subadmin password
  try {
    const subadmins = await db.listSubadmins();
    const subadmin = subadmins.find(sa => db.verifyPassword(password, sa.password_hash));

    if (subadmin) {
      await db.updateSubadminUsername(subadmin.username, new_username);
      return res.json({ success: true, message: 'Username reset successfully' });
    }

    return res.status(401).json({ error: 'Invalid password' });
  } catch (err) {
    console.error('Username reset error:', err);
    return res.status(500).json({ error: 'Failed to reset username' });
  }
});
